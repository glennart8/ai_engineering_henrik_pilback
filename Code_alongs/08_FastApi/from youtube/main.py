...
# JSON serialazation sköts direkt 
# En get-request för alla items
@app.get("/")
...
# Dictionary containing the users query arguments
...
def delete_item(item_id: int) -> dict[str, Item]:
    ...
    if item_id not in items:
        raise HTTPException(
            status_code=404, detals=f"Item with {item_id=} does not exist."
        )
    item = items.pop(item_id)
    return{"deletetd": item}
...
#  ----- REQUESTS ------
# GET-REQUEST - HÄMTAR 
# POST-REQUEST - ADDERAR
# PUT REQUEST - ÄNDRAR/UPPDATERAR
# DELETE REQUEST - TAR BORT